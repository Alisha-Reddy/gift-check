const { keccak256 } = require("ethereum-cryptography/keccak")
const {hexToBytes, bytesToHex} = require("ethereum-cryptography/utils")

const concat = (left, right) => keccak256(Buffer.concat([left, right]));

function verifyProof(proof, leaf, root) {
        //proof - An array of objects whose properties are data and left. (The proof you created in the previous stage)
        // leaf - A leaf node we're trying to prove is within the merkle tree. (Here it's the name we enter in UI)
        // root - The valid Merkle Root
    proof = proof.map(({ data, left }) => ({
        left,
        data: hexToBytes(data), //using map() converting the data of each proof into Bytes
    }));
    let data = keccak256(Buffer.from(leaf));

    for (let i = 0; i < proof.length; i++){
        if (proof[i].left) {
            data = concat(proof[i].data,data)
        } else {
            data = concat(data,proof[i].data)
        }
    }

    return bytesToHex(data) === root;
}

module.exports = verifyProof;
